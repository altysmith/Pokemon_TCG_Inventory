...
The project-folder fallback is **`Start Pokemon Collection.bat`**. It launches the same default-browser experience and has the same automatic shutdown behavior. The desktop shortcut points to the project files, so keep the project in its current Desktop location.
...
Saved decks also appear under **Deck Lists** beside Locations in **My Collection**. Select a deck there to view it, then choose **Edit deck list** to change individual card quantities, remove cards, or search the full local catalog and add another printing. Deck-list changes save to the saved deck only: adding an unowned card is allowed and never changes physical inventory quantities. **Assign owned cards to this deck** marks the available physical copies with the saved deck's name without moving them out of Main Box, a binder, or any other storage location. Once a deck has owned-card assignments, edits made from either My Collection or the full Deck Check automatically refresh those assignments: added entries claim available copies, while removed or reduced entries release copies. Clearing assignments removes only those deck labels.

Selecting a saved deck under **Deck Lists** now shows the complete deck in the main collection area instead of opening the editor immediately. Its artwork tiles use the same visual layout as the collection and are grouped in deck order as **Pokémon**, **Trainer**, then **Energy**. Each tile shows the requested deck quantity and whether the inventory covers it. **Copy deck list** places the original import-format list on the clipboard, **Edit deck list** opens the existing quantity/catalog editor, and **Open full deck check** keeps the full readiness workflow available. The **Full deck list** at the bottom of Deck Check uses the same grouped artwork layout and has its own **Copy deck list** button.
