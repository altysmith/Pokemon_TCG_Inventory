...
**Current build: Iteration 11 — Complete type view**
...
- **Iteration 11 — Complete type view (current):** Keeps every Pokemon elemental group first, then shows Item, Supporter, Tool, Stadium, Basic Energy, and Special Energy as separate groups at the end of the same view.
...
New rows from both workflows are stored in `ocr_reads_it11.csv`. Webcam filenames begin with `webcam_`; every new exact OCR crop is stored under `benchmark_crops/iteration_11`. Earlier iteration files remain untouched as OCR evidence.

The scanner displays the literal OCR result first. It then separates the letters and digit groups into editable fields without correcting them against a known card list. Every scan gets a unique ID. Its exact crop is stored under `benchmark_crops/iteration_11`, while the CSV keeps both the untouched OCR result and your corrected labels. This makes wrong reads, partial reads, and blank reads measurable instead of hiding them.
